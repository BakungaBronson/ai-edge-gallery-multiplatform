          ...
          Column {
            for (rowData in configRows) {
              if (!rowData.isChanged) {
                ...
                val bodySmallNarrowStyle = bodySmallNarrow()
                val annotatedString = buildAnnotatedString {
                  withStyle(style = bodySmallNarrowStyle.toSpanStyle()) {
                    append(rowData.oldValueDisplay)
                  }
                  withStyle(style = bodySmallNarrowStyle.copy(fontSize = 12.sp).toSpanStyle()) {
                    append(" ▸ ") // Added spaces for visual separation
                  }
                  withStyle(
                    style =
                      bodySmallNarrowStyle
